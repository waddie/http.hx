...
Install with [forge](https://github.com/mattwparas/steel), Steel’s package manager. This pulls in the [http2curl](https://github.com/waddie/http2curl.scm) dependency automatically:

```sh
forge pkg install --git https://github.com/waddie/http.hx
```

Then load the plugin in Helix (e.g. add to `init.scm`, see below for an example):

```scheme
(require "http.hx/http-client.scm")
```
